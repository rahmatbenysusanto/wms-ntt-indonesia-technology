...
<html lang="en">
<head>
    <title>Inbound Detail – WMS Mobile</title>
...
<header class="sticky top-0 z-40 bg-brand-400 shadow-md shadow-brand-400/20">
    <div class="flex items-center px-4 h-14">
        <a href="{{ route('inbound.index.mobile') }}" class="flex items-center justify-center w-9 h-9 rounded-xl bg-white/20 text-white">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
        </a>
        <h1 class="absolute left-1/2 -translate-x-1/2 text-white font-bold text-base tracking-wide">Inbound Detail</h1>
...
<main class="px-4 pt-4 space-y-3">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4">
        <div class="flex items-center gap-3 mb-3">
            ...
            <div>
                <p class="font-bold text-slate-800 text-sm">{{ $purchaseOrder->purc_doc }}</p>
                <p class="text-xs text-slate-500">{{ $purchaseOrder->customer->name ?? '-' }}</p>
            </div>
        </div>
        <div class="text-xs text-slate-400 flex items-center gap-1.5">
            ...
            {{ \Carbon\Carbon::parse($purchaseOrder->created_at)->translatedFormat('d F Y H:i') }}
        </div>
    </div>

    {{-- SO List --}}
    <div class="flex items-center justify-between mb-1">
        <h2 class="text-sm font-bold text-slate-700 px-1">Daftar SO</h2>
        <button onclick="document.getElementById('filterModal').classList.remove('hidden')"
            ...
            @if(request()->get('search') == 1)
                <span class="w-2 h-2 rounded-full bg-red-400 ml-0.5"></span>
            @endif
        </button>
    </div>
    @if(request()->get('search') == 1)
        <a href="{{ route('inbound.indexDetail.mobile',['id' => request()->get('id')]) }}"
           class="text-xs text-red-500 px-1 flex items-center gap-1 hover:text-red-700 transition-colors">
    ...
    @forelse($purchaseOrderDetail as $detail)
        <a href="{{ route('inbound.indexDetail.so', ['so' => $detail->sales_doc, 'po' => $purchaseOrder->id]) }}" class="block">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 tap-card
                        {{ $detail->qty == $detail->qtyQc ? 'border-l-4 border-l-emerald-400' : 'border-l-4 border-l-amber-400' }}">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div class="bg-slate-50 rounded-xl p-2 text-center">
                        ...
                        <p class="text-sm font-bold text-slate-700">{{ number_format($detail->qty) }}</p>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-2 text-center">
                        <p class="text-[10px] text-slate-400 font-medium">Product</p>
                        <p class="text-sm font-bold text-slate-700">{{ number_format($detail->qtyProduct) }}</p>
                    </div>
                </div>
            </div>
        </a>
    @empty
        <div class="text-center py-16 text-slate-400">
            <p class="text-sm font-medium">Tidak ada SO ditemukan</p>
        </div>
    @endforelse
</main>
...
<div id="downloadModal" class="hidden fixed inset-0 z-50 flex items-center justify-center px-5">
    ...
    <div class="relative w-full max-w-xs bg-white rounded-3xl p-6 shadow-2xl">
        <h3 class="text-base font-bold text-slate-800 mb-1">Download Laporan</h3>
        <p class="text-xs text-slate-500 mb-5">{{ $purchaseOrder->purc_doc }}</p>
        <div class="space-y-3">
            <a href="{{ route('inbound.purchase-order-download-pdf', ['id' => $purchaseOrder->id]) }}"
            ...
            <a href="{{ route('inbound.purchase-order-download-excel', ['id' => $purchaseOrder->id]) }}"
               class="flex items-center gap-3 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-semibold text-sm rounded-2xl px-4 py-3 transition-all active:scale-[.98]">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3.375 19.5h17.25m-17.25 0a1.125 1.125 0 01-1.125-1.125M3.375 19.5h7.5c.621 0 1.125-.504 1.125-1.125m-9.75 0V5.625m0 12.75v-1.5c0-.621.504-1.125 1.125-1.125m18.375 2.625V5.625m0 12.75c0 .621-.504 1.125-1.125 1.125m1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125m0 3.75h-7.5A1.125 1.125 0 0112 18.375m9.75-12.75c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125m19.5 0v1.5c0 .621-.504 1.125-1.125 1.125M2.25 5.625v1.5c0 .621.504 1.125 1.125 1.125m0 0h17.25m-17.25 0h7.5c.621 0 1.125.504 1.125 1.125M3.375 8.25c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125m17.25-3.75h-7.5c-.621 0-1.125.504-1.125 1.125m8.625-1.125c.621 0 1.125.504 1.125 1.125v1.5c0 .621-.504 1.125-1.125 1.125m-1.5-3.75c.621 0 1.125-.504 1.125-1.125V7.5" /></svg>
                Download Excel
            </a>
        </div>
    </div>
...
<div id="filterModal" class="hidden fixed inset-0 z-50 flex items-end">
    ...
    <div class="relative w-full bg-white rounded-t-3xl p-6 pb-28 shadow-2xl">
        <div class="flex items-center justify-between mb-5">
            <h3 class="text-base font-bold text-slate-800">Cari SO</h3>
            <button onclick="document.getElementById('filterModal').classList.add('hidden')"
                    class="w-8 h-8 flex items-center justify-center bg-slate-100 rounded-full text-slate-500">
