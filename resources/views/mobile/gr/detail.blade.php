...
<html lang="en">
<head>
    <title>GR Detail – WMS Mobile</title>
...
<header class="sticky top-0 z-40 bg-brand-400 shadow-md shadow-brand-400/20">
    <div class="flex items-center px-4 h-14">
        <a href="{{ route('gr.index.mobile') }}" class="flex items-center justify-center w-9 h-9 rounded-xl bg-white/20 text-white">
        ...
        <h1 class="absolute left-1/2 -translate-x-1/2 text-white font-bold text-base">GR Detail</h1>
    </div>
</header>
...
<main class="px-4 pt-4 space-y-3 pb-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4">
        <div class="grid grid-cols-2 gap-1 text-xs mb-3">
            <div><span class="text-slate-400">Purc Doc:</span> <span class="font-semibold text-slate-800">{{ request()->get('po') }}</span></div>
            <div><span class="text-slate-400">Sales Doc:</span> <span class="font-semibold text-slate-800">{{ request()->get('so') }}</span></div>
        </div>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 text-center">
            <div class="w-8 h-8 rounded-lg bg-teal-50 flex items-center justify-center mx-auto mb-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-teal-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10" /></svg>
            </div>
